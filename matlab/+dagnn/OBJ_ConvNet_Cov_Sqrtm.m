classdef OBJ_ConvNet_Cov_Sqrtm < dagnn.Layer
  ...
  methods
    ...
    function outputs = forward(self, inputs, params)
        % %[res(i+1).x, res(i+1).aux_Y, res(i+1).aux_Z] = ConvNet_Cov_Sqrtm(res(i).x); %2017/10/15 06:26
        % [outputs{1} outputs{2} outputs{3}] = ConvNet_Cov_Sqrtm(inputs{1});
        
         % [res(i+1).x, res(i+1).aux{1}, res(i+1).aux{2}] = vl_nncov_sqrtm(res(i).x,    'coef',  l.coef,  'iterNum', l.iterNum,  cudnn{:});
    ...
    function [derInputs, derParams] = backward(self, inputs, params, derOutputs, outputs)
       % % [~, ~, ~,  res(i).dzdx] = ConvNet_Cov_Sqrtm(res(i).x, res(i+1).aux_Y, res(i+1).aux_Z, res(i+1).dzdx); %2017/10/15 06:26
       % % derInputs{1} = ConvNet_Cov_Sqrtm(self, inputs{1}, derOutputs{1}, outputs);
       % [~, ~, ~,  derInputs{1}] = ConvNet_Cov_Sqrtm(inputs{1}, outputs{2}, outputs{3}, derOutputs{1});
       
        %   % res(i).dzdx    = vl_nncov_sqrtm(res(i).x,    res(i+1).aux{1},   res(i+1).aux{2},  res(i+1).dzdx,   'coef', l.coef, 'iterNum', l.iterNum,  cudnn{:});
                 [derInputs{1}] = vl_nncov_sqrtm(inputs{1}, outputs{2},          outputs{3},        derOutputs{1},  'coef', self.coef, 'iterNum', self.iterNum,  self.cudnn{:});
