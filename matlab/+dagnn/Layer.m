classdef Layer < handle
    ...
    methods
        ...
        function forwardAdvanced(obj, layer)
            ...
            GT = net.getVarIndex('gating');
            %       inputs = {net.vars(in).value} ;
            if ~strncmp(layer.name , 'CM',2) | isempty(strfind(layer.name , 'conv'))...
                    | isnan(GT) 
                % if current layer is not the conv. layer in CM , go as previous
                inputs = {net.vars(in).value} ;
            else
                % if current layer is conv. layer in CM , features of few 
                % samples are selected into CM  in which their weights are ranked in topk
                uu = strfind(layer.name , '_');
                CM_ID  =str2double( ...
                    layer.name(uu(1) + 1 : uu(2) -1));%get ID. of CM
                if numel(uu) == 3 & ~strcmp(layer.name(uu(2) + 1), 'a')
                    inputs = {net.vars(in).value} ;%layer CM_i_b_conv receives whole featuremap of CM_i_a_relu directly
                else %CM_i_a_conv gets samples
                    %get the sample id which selects this CM
                    img_num = net.vars(GT).value{2}(CM_ID , :);
                    
                    %get features which need encoding by this CM according to samples id
                    inputs = {net.vars(in).value(:,:,:,img_num)} ;
                end
            end
        ...
        function backwardAdvanced(obj, layer)
            ...
            %       inputs = {net.vars(in).value} ;
            GT = net.getVarIndex('gating');
            if ~strncmp(layer.name , 'CM',2) ...
                    | isempty(strfind(layer.name , 'conv')) | isnan(GT)
                % if current layer is not the conv. layer in CM , go as previous                
                inputs = {net.vars(in).value} ;
                CM_flag = false;
            else                
                % if current layer is conv. layer in CM , gradient of few 
                % samples are selected into CM  in which their weights are ranked in topk
                uu = strfind(layer.name , '_');
                CM_ID  =str2double( layer.name(uu(1) + 1 : uu(2) -1));
                if numel(uu) == 3 & ~strcmp(layer.name(uu(2) + 1), 'a')
                    inputs = {net.vars(in).value} ;
                    CM_flag = false;
                else
                    img_num = net.vars(net.getVarIndex('gating')).value{2}(CM_ID , :);
                    inputs = {net.vars(in).value(:,:,:,img_num)} ;
                    CM_flag = true;
                end
            end
            ...
            if CM_flag
                % setting derivative of unselected samples to zeros
                dd = derInputs{1};
                derInputs = [];
                s  = size(dd);
                s(4) = size(img_num , 2);
